...
class ActivePlacesAdapter(private var historyPlacess: ActivePlaces) : RecyclerView.Adapter<ActivePlacesAdapter.PlacesHolder>() {
    ...
    fun filter() {
        historyPlaces.clear()
        SourceListHistory.forEach({

            var calendar = Calendar.getInstance()
            var nowDate = "" + calendar.get(Calendar.DAY_OF_MONTH) + " " + (calendar.get(Calendar.MONTH) + 1) + " " + calendar.get(
                Calendar.YEAR)

            val sdf = SimpleDateFormat("dd MM yyyy")
            val nowDateConv = sdf.parse(nowDate)
            calendar.time = nowDateConv
